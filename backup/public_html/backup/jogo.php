<?php
/* Faça um jogo da velha com PHP. O jogo será com dois jogadores. Se houver
...
if (isset($_GET['nomeum'])) {
    // se existir nome um, passar todos os get do corpo do jogo pra caixa alta facilita na comparação...
    for ($i = 0; $i < 9; $i++) {
        $_GET['corpo' . $i] = strtoupper($_GET['corpo' . $i]); // => letras maiúsculas uppercase
    }
    $nomeum = $_GET['nomeum']; // variável que armazena o nome do jogador recebe get
} else {
    $nomeum = ""; // variável nome recebe vazio, não existe o get...
}
...
for ($k = 0; $k < 9; $k++) {
    if (isset($_GET['corpo' . $k]) && !empty($_GET['corpo' . $k])) {
        if ($_GET['corpo' . $k] == 'X' || $_GET['corpo' . $k] == 'O') {
            ...
            $resp[$k] = "";
            echo "<script>alert('caractere inválido, use apenas O ou X...');</script>";
            $_GET['corpo' . $k] = "";
        }
    } else {
        $resp[$k] = "";
...
$venceu = ""; // variável onde será armazenado o caractere do vencedor
// central
...
/* Guia rápido para entender o objeto DateTime
...
<head>
    ...
    <style type="text/css">
        ...
        div#jogo {
            background: url("fundo.png") no-repeat center;
        }
        .bot {
            border-radius: 10px;
            padding: 5px;
...
<body>
    <form method="get" action="#">
        <div id="cabecalho">
            ...
            0 <input type="text" placeholder="Entre com o nome do jogador" name="nomeum" value="<?= $nomeum; ?>" class="jognome">&nbsp;0&nbsp;&nbsp;&nbsp;&nbsp;X<input class="jognome" type="text" placeholder="Entre com o nome do jogador" name="nomedois" value="<?= $nomedois; ?>">X
            <div id="jogo">
                <?php for ($k = 0; $k < 9; $k++): ?>
                    <?php if ($k % 3 == 0): ?><!-- a cada três input um br -->
                        <br />
                    <?php endif; ?>
                    <input size="1" type="text" name="corpo<?= $k; ?>" class="jg" value="<?= $resp[$k]; ?>">
                <?php endfor; ?>
            </div>
            <input type="submit" name="botao" value="Jogar" class="bot"> &nbsp; &nbsp; &nbsp; <a href="index.php">Resetar</a>
            <p><?= $tempo; ?></p>
        </div>
    </form>
</body>
